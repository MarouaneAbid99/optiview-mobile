...
export function SignupScreen({ navigation }) {
  ...
  const [form, setForm] = useState({ name: "", shopName: "", email: "", password: "", confirmPw: "" });
  ...
  const onSubmit = async () => {
    setError("");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) { setError("Email invalide"); return; }
    ...
    setLoading(true);
    try {
      await register({ name: form.name, shopName: form.shopName, email: form.email.trim(), password: form.password });
    } catch (e) {
    ...
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: colors.bg }}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          {[
            { k: "name", label: "Votre nom", icon: "person-outline", kbType: undefined, autoC: "words" },
            { k: "shopName", label: "Nom de la boutique", icon: "storefront-outline", kbType: undefined, autoC: "words" },
            { k: "email", label: "Email", icon: "mail-outline", kbType: "email-address", autoC: "none" },
          ].map(({ k, label, icon, kbType, autoC }) => (
            <View key={k}>
              ...
              <View style={styles.inputWrap}>
                ...
                <TextInput style={styles.input} value={form[k]} onChangeText={(v) => set(k, v)}
                  autoCapitalize={autoC} keyboardType={kbType} placeholderTextColor={colors.mutedLight} />
              </View>
            </View>
          ))}
          ...
          <View style={styles.inputWrap}>
            <Ionicons name="lock-closed-outline" size={17} color={colors.muted} style={styles.inputIcon} />
            <TextInput style={[styles.input, { flex: 1 }]} value={form.confirmPw} onChangeText={(v) => set("confirmPw", v)}
              secureTextEntry={!showConfirm} placeholder="Répétez le mot de passe" placeholderTextColor={colors.mutedLight} />
